...
const pool = new Pool({
  ...
  database: process.env.PGDATABASE || 'flowvibe',
...
async function initDb() {
  try {
    ...
    const { rows } = await query('SELECT COUNT(*) FROM pricing_plans');
    ...
    const tierCount = await query('SELECT COUNT(*) FROM custom_tiers');
