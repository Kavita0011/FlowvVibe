...
async function handleRegister(request, env) {
  ...
  const existingResult = await queryNeon(env, `SELECT id FROM "users" WHERE LOWER(email) = LOWER($1)`, [email]);
...
async function handleForgotPassword(request, env) {
  try {
    ...
    if (env.NEON_DATABASE_URL) {
      const userResult = await queryNeon(env, "/* Using backticks (`` ` ``) fixes the syntax error*/ queryNeon(env, `SELECT id FROM "users" WHERE LOWER(email) = LOWER($1)`, [email]);
